...
develop=False
pre_list=       ['7','10','11','15',   '20','22','23','24','25',   '28','29','30','31','32']
...
def prepare_longdistance(msafile,savefile_):
    ...
    pcb_10 = (re_dic['22'][0]*ensemble_weight['22'] + re_dic['29'][0]*ensemble_weight['29'])/(ensemble_weight['29']+ensemble_weight['22'])
    pcb_13 = (re_dic['23'][0]*ensemble_weight['23'] + re_dic['30'][0]*ensemble_weight['30'])/(ensemble_weight['23']+ensemble_weight['30'])
    pcb_16 = (re_dic['24'][0]*ensemble_weight['24'] + re_dic['31'][0]*ensemble_weight['31'])/(ensemble_weight['24']+ensemble_weight['31'])
    pcb_19 = (re_dic['25'][0]*ensemble_weight['25'] + re_dic['32'][0]*ensemble_weight['32'])/(ensemble_weight['25']+ensemble_weight['32'])

    pca_20 = (re_dic['11'][-2]*ensemble_weight['11'] + re_dic['20'][1]*ensemble_weight['20'] + re_dic['28'][-2]*ensemble_weight['28'])/ \
             (ensemble_weight['20']+ensemble_weight['28']+ensemble_weight['11'])   
    pca_10 =  re_dic['29'][1]
    pca_13 =  re_dic['30'][1]
    pca_16 =  re_dic['31'][1]
    pca_19 =  re_dic['32'][1]
    ...
    cb_contact=(np.sum(pcb_20[0][:13],axis=0)*2.0+np.sum(pcb_13[0][:13],axis=0)*2.0+np.sum(pcb_16[0][:13],axis=0)*2.0+np.sum(pcb_10[0][:13],axis=0)*2.5 +\
        np.sum(pcb_19[0][:13],axis=0)*1.5) / (2.0+2.0+2.0+2.5+1.5)

    ca_contact=(np.sum(pca_20[0][:13],axis=0)+np.sum(pca_13[0][:13],axis=0)+np.sum(pca_16[0][:13],axis=0)+np.sum(pca_10[0][:13],axis=0) +\
        np.sum(pca_19[0][:13],axis=0) )/ (5.0)

    out_npz(pcb_20,pomg_20,ptheta_20,pphi_20,savefile_+'_20.npz')
    npytotxt(pcb_20,savefile_+'_pcb_20.txt')
    npytotxt(pcb_10,savefile_+'_pcb_10.txt')
    npytotxt(pcb_13,savefile_+'_pcb_13.txt')
    npytotxt(pcb_16,savefile_+'_pcb_16.txt')
    npytotxt(pcb_19,savefile_+'_pcb_19.txt')

    npytotxt(pca_20,savefile_+'_pca_20.txt')
    npytotxt(pca_10,savefile_+'_pca_10.txt')
    npytotxt(pca_13,savefile_+'_pca_13.txt')
    npytotxt(pca_16,savefile_+'_pca_16.txt')
    npytotxt(pca_19,savefile_+'_pca_19.txt')

    npytotxt(paa_10,savefile_+'_paa_10.txt')
    npytotxt(pbb_10,savefile_+'_pbb_10.txt')
    npytotxt(pcc_10,savefile_+'_pcc_10.txt')

    npytotxt(paa_20,savefile_+'_paa_20.txt')
    npytotxt(pbb_20,savefile_+'_pbb_20.txt')
    npytotxt(pcc_20,savefile_+'_pcc_20.txt')

    npytotxt(pomg_20,savefile_+'_pomg_20.txt')
    npytotxt(pphi_20,savefile_+'_pphi_20.txt')
    npytotxt(ptheta_20,savefile_+'_ptheta_20.txt')




    npytotxt(ca_contact[None,None,:,:],savefile_+'_ca_contact.txt')
    npytotxt(cb_contact[None,None,:,:],savefile_+'_cb_contact.txt')
