...
class preNet(nn.Module):
    def __init__(self,featurenum2d,featurenum1d,dis_dim,theta_dim,phi_dim,omg_dim,hbp_dim,do_attention):
        ...
        self.conv_tran1=nn.Conv2d(featurenum2d,self.hidden_channel,kernel_size=1, bias=False)#.cuda(0)
        self.bn_tran1=nn.InstanceNorm2d(self.hidden_channel)#.cuda(0)

        self.conv_tran2=nn.Conv2d(self.hidden_channel+self.hidden_channel1d*2,self.hidden_channel,kernel_size=1, bias=False)#.cuda(0)
        self.bn_tran2=nn.InstanceNorm2d(self.hidden_channel)#.cuda(0)

        self.conv_tran1d=nn.Conv1d(featurenum1d,self.hidden_channel1d,kernel_size=1, bias=False)#.cuda(0)
        self.bn_tran1d=nn.InstanceNorm1d(self.hidden_channel1d)#.cuda(0)

        self.res1=self._make_layer(BasicBlock,self.blocks1[0],self.kernel_sizes[:],self.dialations[:])#.cuda(0)
        self.res2=self._make_layer(BasicBlock,self.blocks2[0],self.kernel_sizes[:],self.dialations[:])#.cuda(0)
        self.res1d=self._make_layer1d(BasicBlock1d,self.blocks1d[0],self.kernel_sizes[:],self.dialations[:])#.cuda(0)


        
        self.conv_cb=nn.Conv2d(self.hidden_channel,dis_dim,3,padding=1)#.cuda(0)
        self.conv_omg=nn.Conv2d(self.hidden_channel,omg_dim,1,padding=0)#.cuda(0)
        self.conv_hbp=nn.Conv2d(self.hidden_channel,hbp_dim,3,padding=1)#.cuda(0)
        # un sys
        self.conv_theta=nn.Conv2d(self.hidden_channel,theta_dim,1,padding=0)#.cuda(0)
        self.conv_phi=nn.Conv2d(self.hidden_channel,phi_dim,1,padding=0)#.cuda(0)


                   
        self.relu = nn.ReLU(inplace=True)
        self.logsoft=nn.Softmax(1)
