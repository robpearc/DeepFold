...
double **** new4DArr(int dim1, int dim2, int dim3, int dim4){
	double ****ans;
	ans = new double ***[dim1];
	for(int i=0; i<dim1; i++){
		ans[i] = new double **[dim2];
	}	
	for(int i=0; i<dim1; i++){
		for(int j=0; j<dim2; j++){
			ans[i][j] = new double *[dim3];
		}
	}
	for(int i=0; i<dim1; i++){
		for(int j=0; j< dim2; j++){
			for(int k=0; k<dim3; k++){
				ans[i][j][k] = new double [dim4];
			}
		}
	}
...
void release4DArr(int dim1, int dim2, int dim3, double **** Arr){
	if(!Arr) return;
	for(int t=0; t<dim1; t++){
		for(int i=0; i<dim2; i++)
			for(int j=0; j<dim3; j++)
