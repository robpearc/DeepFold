...
#include "cs/cs.h"
#include "cs/alignment-inl.h"
#include "cs/application.h"
#include "cs/blosum_matrix.h"
#include "cs/context_library.h"
#include "cs/count_profile-inl.h"
#include "cs/getopt_pp.h"
#include "cs/library_pseudocounts-inl.h"
#include "cs/matrix_pseudocounts-inl.h"
#include "cs/pssm.h"
#include "cs/sequence-inl.h"
...
struct CSTranslateAppOptions {
    ...
    void Init() {
        ...
        else
        {
          ...
          {
            std::cerr<<"WARNING! Cannot find  executable folder."<<std::endl;
        ...
        modelfile   =(string)(hhlib)+"/data/context_data.lib";
        alphabetfile=(string)(hhlib)+"/data/cs219.lib";
    }
...
void CSTranslateApp<Abc>::ParseOptions(GetOpt_pp& ops) {
    ...
    if (opts_.outfile.empty() && opts_.appendfile.empty())
        opts_.outfile = GetBasename(opts_.infile, false) + ".seq219";
...
void CSTranslateApp<Abc>::PrintOptions() const {
    fprintf(out_, "  %-30s %s\n", "-i, --infile <fileList>",
            "List of input file with alignment or sequence");
    fprintf(out_, "  %-30s %s\n", "-o, --outfile <fileList>", "List of output file for generated abstract state sequence (def: <infile>.seq219)");
    ...
    fprintf(out_, "\n\nFor both -i/--infile and -o/--outfile, the file name extension of the list and those of the listed files must be consistent.\n"
                  "For example, if --infile <fileList> list *.a3m files, the name for the <fileList> file itself must also end with .a3m.\n");
}
...
int CSTranslateApp<Abc>::Run() {
    ...
    std::string line;
    std::ifstream fp;
    fp.open(opts_.infile.c_str(),std::ios::in);
    while (fp.good())
    {
        getline(fp,line);
    ...
    fp.close();
    fp.open(opts_.outfile.c_str(),std::ios::in);
    while (fp.good())
    {
        getline(fp,line);
    ...
    {
        std::cerr<<"ERROR! "<<opts_.infile<<" and "
            <<opts_.outfile<<" do not have the same length"<<std::endl;
    }

    for (size_t f=0;f<inList.size();f++)
    {
        opts_.infile=inList[f];
        opts_.outfile=outList[f];
        string header;
    ...
    std::vector <std::string>().swap(inList);
    std::vector <std::string>().swap(outList);
    line.clear();
    return 0;
}
