...
void help(char all=0)
{
  ...
  printf(" -i <fileList>  list of query alignment (A2M, A3M, or FASTA), or query HMM\n");
  printf("\n");
  printf("Output options:                                                           \n");
  printf(" -o <fileList>  list of HMM file to be written to  (default=<infile.hhm>) \n");
...
int main(int argc, char **argv)
{
  ...
  {
    std::cerr<<"ERROR! "<<par.infile<<" and "
      <<par.outfile<<" do not have the same length"<<std::endl;
  }
  ...
  {
    strcpy(par.infile, inList[f].c_str());
    strcpy(par.outfile,outList[f].c_str());
