...
Options:
  ...
  -batch    batch convert files. <infile> and <outfile> must be each be a list of input and
            output files.
...
Examples: reformat.pl 1hjra.a3m 1hjra.a2m  
          ...
          reformat.pl fas a3m input_fasta.list output_a3m.list -batch
\n");
...
my $gap="default";
my $case="default";
my $batch=0; # convert individual files
...
{
    my @infiles=`cat $infile`;
    my @outfiles=`cat $outfile`;
    my $filenum=scalar(@infiles);
    if (scalar(@outfiles)!=$filenum)
    { die("Error: the numbers of input and output files are inconsistent\n"); }
    printf("$filenum files to reformat\n");
    for (my $f=0;$f<$filenum;$f++)
    {
        $infile =$infiles[$f];
        $outfile=$outfiles[$f];
        chomp($infile);
