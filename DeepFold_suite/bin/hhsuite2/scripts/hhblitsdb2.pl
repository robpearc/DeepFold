...
#
#     This script is based on hhblitsdb.pl from HHsuite version 2.0.16
#
#     Reference: 
...
my $cpu = 0;          # do not use multi-threading
...
Options:
 ...
 -cpu <int>      number of threads to generate cs219 and hhm files (default = $cpu)
                 Due to I/O reason, -cpu 0, which disables multithreading,
                 often generate results much faster.
...
if (!-d dirname($dbname)) { die("ERROR! Parent folder of database does not exist. Create it with\nmkdir -p ".dirname($dbname)."\n"); }
$a3mdir=~s/\/$//; 
...
##############################################################################################
	...
	if ($a3mdir) {
	    print("Generating seq219 files in $tmpdir/ from a3m files $a3mdir\n\n");
            # context_lib=$HHLIB/context_data.lib
            # cs_lib=$HHLIB/data/cs219.lib
            if ($cpu>=1)
            {
            ...
            {
                open(FP_a3m,">$dbname\_list.a3m");
                open(FP_seq219,">$dbname\_list.seq219");
                foreach my $filename (glob($a3mdir))
                {
                    chomp($filename);
                    print FP_a3m "$filename\n";
                    $filename=basename($filename);
                    if ($filename =~/(.*)\..*?$/) { $filename=$1;}
                    print FP_seq219 "$tmpdir/$filename.seq219\n";
                }
                close(FP_a3m);
                close(FP_seq219);
                $command = "$hhbin/cstranslate_batch -i $dbname\_list.a3m -o $dbname\_list.seq219 >> $logfile 2>&1";
                &HHPaths::System("$command");
                &HHPaths::System("rm $dbname\_list.a3m $dbname\_list.seq219");
            }
	    ...
	    print("\nGenerating hhm files in $tmpdir/ from a3m files $a3mdir/\n\n");
            ...
            else
            {
                ...
                foreach my $filename (glob($a3mdir))
                {
                    ...
                    $filename=basename($filename);
                    if ($filename =~/(.*)\..*?$/) { $filename=$1;}
                    print FP_hhm "$tmpdir/$filename.hhm\n";
                }
                close(FP_a3m);
                close(FP_hhm);
                $command = "$hhbin/hhmake_batch -i $dbname\_list.a3m -o $dbname\_list.hhm 1>/dev/null >> $logfile";
                &HHPaths::System("$command");
                &HHPaths::System("rm $dbname\_list.a3m $dbname\_list.hhm");
            }
